...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        for test_ip, test_label in test_data_loader:
            test_label = test_label.to(device)
            mask = test_ip['attention_mask'].to(device)
            input_id = test_ip['input_ids'].squeeze(1).to(device)

            output = model(input_id, mask)

            print(enc_labels.classes_[output.argmax(dim=1)])